...
public class PiperSubscriptionService {
    ...
    public void unsubscribe(String userId, String publishTemail, PublisherTypeEnums ptype) {
        if (StringUtils.isNullOrEmpty(psUtil.publickey(publishTemail))) {
...
//		psClientService.sendTextMessage(publisher.getName() + "<" + publisher.getPtemail() + "> 订阅成功，作者即将推送文章", userId, 0);
        ...
        //给创建者发个消息
        messegerSenderService.sendSynchronizationTxt(publisher.getPtemail(), userId, languageChange.getLangByUserId("msg.submotice", new String[]{userId}, publisher.getUserId()));
