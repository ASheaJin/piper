...
public class RecommendInnerController {
	...
	public ResponseEntity publisherList(@RequestBody RecommendInput ri, HttpServletRequest request) {
	...
	public ResponseEntity contentList(@RequestBody RecommendInput ri, HttpServletRequest request) {
		Integer pageNo = StringUtils.isNullOrEmpty(ri.getPageNo()) ? 1 : Integer.parseInt(ri.getPageNo());
		Integer pageSize = StringUtils.isNullOrEmpty(ri.getPageSize()) ? 20 : Integer.parseInt(ri.getPageSize());


		String manageId = headerService.getUserId(request);
		PageInfo<ReCommendContent> pageInfo = piperRecommendContentService.list(manageId, pageNo, pageSize);
